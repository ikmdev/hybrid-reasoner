...
public class SubsumptionNormalFormBuilder {
	...
	private HashMap<OWLClass, List<SubsumptionNormalForm>> unmergedSubsumptionNormalFormsSHIELD = new HashMap<>();
	private HashMap<OWLClass, SubsumptionNormalForm> subsumptionNormalFormsSHIELD = new HashMap<>();
	...
	public List<SubsumptionNormalForm> getUnmergedSubsumptionNormalFormsSHIELD(OWLClass con) {
		return unmergedSubsumptionNormalFormsSHIELD.get(con);
	}

	public HashMap<OWLClass, SubsumptionNormalForm> getSubsumptionNormalFormsSHIELD() {
...
//		for (OWLObjectProperty prop : kernelOntology.getObjectPropertiesInSignature()) {
			 ...
			 * handle them, given above Set<OWLObjectPropertyExpression> propsSuperProps =
...
// Needed??	Reasoner.processingSubsumptionNormalFormSHIELD = false;
	...
	private SubsumptionNormalForm mergeNNFs(List<SubsumptionNormalForm> nnfs) {
...
//			default -> throw new UnsupportedOperationException(
	...
	@SuppressWarnings("deprecation")
	private Set<OWLObjectSomeValuesFrom> processIntersectionProperties(Set<OWLClassExpression> class_exprs,
			OWLClassAxiom axiom) {
		Set<OWLObjectSomeValuesFrom> properties = new HashSet<>();
...
//				.toList();
		Stream<SVF> superPropsStream = (Stream<SVF>) superProps.get(svf.prop).stream().filter(x -> !x.equals(svf.prop))
...
//	public static final String ROLE_GROUP_PROPERTY_NAME)
	...
	private boolean containsAbsentProperty(Set<OWLObjectSomeValuesFrom> ungroupedProps,
		...
		for (OWLObjectSomeValuesFrom prop : ungroupedProps) {
			if (prop.getProperty().getNamedProperty().getIRI().equals(roleGroupPropertyIri)) {
				...
				if (propertyValue instanceof OWLObjectSomeValuesFrom) {
					...
					Set<OWLClassExpression> intersectedClassExpressions = ((OWLObjectIntersectionOf) propertyValue)
							.getOperands();
	 ...
	 * "http://www.hhs.fda.org/shield/SWEC-Ontology" absenceValueName = "Absent"
	 * 
	 ...
	 * 
	 * <ObjectSomeValuesFrom> <ObjectProperty IRI=
	 * "http://www.hhs.fda.org/shield/SWEC-Ontology#Situation-Presence"/>
